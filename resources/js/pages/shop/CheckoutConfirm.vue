...
function selectPlan(p: Plan) { selectedPlanName.value = p.name; showPlanModal.value = false }
...
const selectedPeriod = ref<Period>(
    PERIODS.find(p => p.months === Number(props.billingMonths)) ?? PERIODS[1]
)
...
// ── Accent + prefix insensitive match ─────────────────────────────────────
// Normalizes both strings:
//   - NFD decomposition strips accents (ñ → n, é → e)
//   - strips "City of " prefix so "Dasmariñas" matches "City of Dasmariñas"
//   - lowercases + trims
// This handles: encoding differences, "City of" prefix, ñ vs n, Ã±, etc.
function fuzzyMatch(a: string, b: string): boolean {
    const norm = (s: string) =>
        s.normalize('NFD')
         .replace(/[\u0300-\u036f]/g, '')   // strip combining diacritics
         .replace(/^city of\s+/i, '')        // strip "City of " prefix
         .trim()
         .toLowerCase()
...
onMounted(async () => {
    ...
    try {
        ...
        if (props.user.municipality) {
            const saved = props.user.municipality.trim()
            // ✅ fuzzyMatch handles: "Cavite City" == "City of Cavite",
            //    "Dasmariñas" == "City of Dasmariñas", encoding differences, etc.
            const match = cityMunis.value.find(m => fuzzyMatch(m.name, saved))

            if (match) {
                selectedCityMuni.value = match.code   // triggers watcher → loads barangays
            } else {
                console.warn('[Checkout] No municipality match for:', JSON.stringify(saved))
                console.warn('[Checkout] Available:', cityMunis.value.map(m => m.name))
...
watch(selectedCityMuni, async (code) => {
    ...
    try {
        ...
        if (isInitialLoad.value && props.user.barangay) {
            const savedBrgy = props.user.barangay.trim()
            // ✅ fuzzyMatch handles accent differences in barangay names too
            const match = barangays.value.find(b => fuzzyMatch(b.name, savedBrgy))
            if (match) {
                selectedBarangay.value = match.code
            } else {
                console.warn('[Checkout] No barangay match for:', JSON.stringify(savedBrgy))
                console.warn('[Checkout] Available:', barangays.value.map(b => b.name))
            }
        }
    } catch (e) {
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_320px]">
                ...
                <div class="space-y-5">
                    ...
                    <div class="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
                        <p class="mb-3 text-[11px] font-semibold uppercase tracking-widest text-stone-400">Selected Plan</p>
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-semibold text-stone-800">{{ plan.name }} — {{ selectedPeriod.fullLabel }}</span>
                            <button type="button"
                    ...
                    <div class="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
                        ...
                        <div v-if="apiError"
                            ...
                            <AlertCircle class="h-3.5 w-3.5 shrink-0" />{{ apiError }}
                        </div>
                        ...
                        <div class="grid gap-4">
                            <div>
                                ...
                                <Input v-model="shopForm.shop_name" placeholder="Laundry Express" class="border-stone-200 bg-stone-50 focus:bg-white" />
                            </div>
                            ...
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div>
                                    ...
                                    <Input v-model="shopForm.phone" placeholder="09XX-XXX-XXXX" class="border-stone-200 bg-stone-50 focus:bg-white" />
                                </div>
                                <div>
                                    <Label class="mb-1.5 flex items-center gap-1.5 text-xs font-medium text-stone-600">
                                        <MapPin class="h-3.5 w-3.5" /> Block / Street <span class="text-red-400">*</span>
                                    </Label>
                                    <Input v-model="shopForm.block_street" placeholder="e.g. Block 5, St. 12" class="border-stone-200 bg-stone-50 focus:bg-white" />
                                </div>
                            </div>
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        <select v-model="selectedCityMuni"
                                            ...
                                            <option v-for="m in cityMunis" :key="m.code" :value="m.code">{{ m.name }}</option>
                                        </select>
                                        <Loader2 v-if="loadingCityMunis" class="pointer-events-none absolute right-2.5 top-2.5 h-4 w-4 animate-spin text-stone-400" />
                                        <svg v-else viewBox="0 0 24 24" fill="none" stroke="currentColor" class="pointer-events-none absolute right-2.5 top-2.5 h-4 w-4 text-stone-400">
                                    ...
                                    <div class="relative">
                                        <select v-model="selectedBarangay"
                                            ...
                                            <option value="">
                                                ...
                                                <template v-else-if="!selectedCityMuni">Select a city/municipality first</template>
                                                <template v-else>— Select Barangay —</template>
                                            </option>
                                            <option v-for="b in barangays" :key="b.code" :value="b.code">{{ b.name }}</option>
                                        </select>
                                        <Loader2 v-if="loadingBarangays" class="pointer-events-none absolute right-2.5 top-2.5 h-4 w-4 animate-spin text-stone-400" />
                                        <svg v-else viewBox="0 0 24 24" fill="none" stroke="currentColor" class="pointer-events-none absolute right-2.5 top-2.5 h-4 w-4 text-stone-400">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                                        </svg>
                                    </div>
                            ...
                            <div class="w-1/3">
                                <Label class="mb-1.5 text-xs font-medium text-stone-600">Postal Code <span class="text-red-400">*</span></Label>
                                <Input v-model="shopForm.postal_code" placeholder="4102" class="border-stone-200 bg-stone-50 focus:bg-white" />
                    ...
                    <div class="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
                        ...
                        <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
                            <button v-for="method in PAYMENT_METHODS" :key="method.key" type="button"
                                ...
                                <span v-if="method.tag" class="absolute right-2 top-2 rounded-full bg-blue-100 px-1.5 py-0.5 text-[10px] font-semibold text-blue-600">
                                    {{ method.tag }}
                                </span>
                            </button>
                        </div>
                        ...
                        <div class="space-y-3">
                            <div v-for="(meta, key) in kycMeta" :key="key" class="rounded-xl border transition-colors"
                                ...
                                <div v-if="kycDocs[key]" class="flex items-center gap-3 p-3.5">
                                    ...
                                    <button type="button" class="flex h-6 w-6 items-center justify-center rounded-full text-stone-400 transition-colors hover:bg-stone-200 hover:text-stone-600" @click="removeFile(key)">
                                        <X class="h-3.5 w-3.5" />
                                    </button>
                                </div>
                                <label v-else :for="`kyc-${key}`" class="flex cursor-pointer items-center gap-3 p-3.5">
                                    ...
                                    <div class="flex-1">
                                        <p class="text-sm font-medium text-stone-700">{{ meta.label }}<span v-if="meta.required" class="ml-0.5 text-red-400">*</span></p>
                                        <p class="text-xs text-stone-400">{{ meta.description }}</p>
                                        <p class="mt-0.5 text-xs text-blue-500">Click to upload · PDF, JPG, PNG</p>
                                    </div>
                                    <input :id="`kyc-${key}`" type="file" accept=".pdf,.jpg,.jpeg,.png" class="sr-only" @change="handleFileUpload($event, key)" />
                                </label>
                            </div>
                        </div>
                ...
                <div>
                    <div class="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm lg:sticky lg:top-6">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between text-stone-600"><span>Monthly price</span><span class="font-medium text-stone-800">{{ fmt(discountedMonthly) }}</span></div>
                            <div class="flex justify-between text-stone-600"><span>Duration</span><span class="font-medium text-stone-800">× {{ selectedPeriod.months }} mo</span></div>
                            <div class="flex justify-between text-stone-600"><span>Subtotal</span><span class="font-medium text-stone-800">{{ fmt(subtotal) }}</span></div>
                            <div class="flex justify-between text-stone-600"><span>VAT ({{ props.vatPct }}%)</span><span class="font-medium text-stone-800">{{ fmt(vatAmount) }}</span></div>
                            <div v-if="savedAmount > 0" class="flex justify-between font-medium text-emerald-600"><span>You save</span><span>− {{ fmt(savedAmount) }}</span></div>
                        </div>
                        <hr class="my-4 border-stone-100" />
                        <div v-if="selectedPayment" class="mb-4 flex items-center gap-2 rounded-lg border border-stone-100 bg-stone-50 px-3 py-2">
                        ...
                        <div v-if="!canProceed" class="mt-4 flex items-center gap-2 rounded-lg bg-amber-50 p-2.5 text-xs text-amber-700">
                        ...
                        <Button class="mt-4 w-full bg-blue-500" type="button" :disabled="!canProceed || isSubmitting" @click="proceedToPayment">
                            <Loader2 v-if="isSubmitting" class="mr-2 h-4 w-4 animate-spin" />
                            {{ isSubmitting ? 'Processing...' : 'Proceed to Payment' }}
                        </Button>
                        <div class="mt-4 flex items-center justify-center gap-5">
                            <span class="flex items-center gap-1.5 text-xs text-stone-400"><ShieldCheck class="h-3.5 w-3.5" /> Secure</span>
                            <span class="flex items-center gap-1.5 text-xs text-stone-400"><Clock class="h-3.5 w-3.5" /> Instant activation</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-if="showPlanModal"
                ...
                <div class="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl">
                    <div class="mb-5 flex items-center justify-between">
                        <h2 class="text-base font-semibold text-stone-900">Change Plan & Billing Period</h2>
                        <button type="button" class="flex h-7 w-7 items-center justify-center rounded-full text-stone-400 hover:bg-stone-100" @click="showPlanModal = false">
                            <X class="h-4 w-4" />
                        </button>
                    </div>
                    <p class="mb-3 text-[11px] font-semibold uppercase tracking-widest text-stone-400">Plan</p>
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
                        <button v-for="p in planList" :key="p.name" type="button" @click="selectPlan(p)"
                            ...
                            <p class="mb-3 text-lg font-bold text-stone-900">{{ fmt(p.price) }}<span class="text-xs font-normal text-stone-400">/mo</span></p>
                            <ul class="space-y-1">
                                <li v-for="mod in p.moduleNames" :key="mod" class="flex items-center gap-1.5 text-xs text-stone-600">
                                    <CheckCircle2 class="h-3 w-3 shrink-0 text-emerald-500" />{{ mod }}
                                </li>
                            </ul>
                        </button>
                    </div>
                    <p class="mt-5 mb-3 text-[11px] font-semibold uppercase tracking-widest text-stone-400">Billing Period</p>
                    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                        <button v-for="period in PERIODS" :key="period.months" type="button"
                            ...
                            <span v-if="period.discountPct > 0" class="absolute -top-2.5 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-emerald-500 px-2 py-0.5 text-[10px] font-semibold text-white">
                                Save {{ period.discountPct }}%
                            </span>
                            <span class="text-sm font-semibold" :class="selectedPeriod.months === period.months ? 'text-blue-600' : 'text-stone-800'">
                                {{ period.shortLabel }}
                            </span>
                            <span class="mt-1 text-xs text-stone-400">{{ fmt(Math.round(plan.price * (1 - period.discountPct / 100))) }}/mo</span>
                        </button>
                    </div>
                    <p class="mt-4 text-center text-xs text-stone-400">Changes apply immediately to your order summary.</p>
                </div>
            </div>
        </Teleport>
    </div>
